...
class DataCollector {
  constructor() {
    this.linkTemplates = {
      files_list: {
        base: this.linkData.baseAppLink,
        path: 'sling/info/:appNum',
      },
      ipoteka_lien: {
        base: this.linkData.baseMappLink,
        path: 'lien/:appNum/estate-data',
      },
      ipoteka_history: {
        base: this.linkData.baseMappLink,
        path: 'history-real-estate/:appNum',
      },
      ipoteka_info_mode: {
        base: this.linkData.baseMappLink,
        path: 'credit-request/:appNum/application/personal-data?mode=info',
      },
      ipoteka_view_mode: {
        base: this.linkData.baseMappLink,
        path: ':appNum?mode=view',
      },
      ipoteka_final_view_mode: {
        base: this.linkData.baseMappLink,
        path: 'contract/:appNum/final?mode=info',
      },
      potreb_redir: {
        base: this.linkData.baseAppLink,
        path: ':appNum',
      ...
      cc_redir: {
        base: this.linkData.baseCardLink,
        path: ':appNum',
      },
      auto_redir: {
        base: this.linkData.baseAutoLink,
        path: ':appNum',
      },
    };
  }
  ...
  makeLinkOpt = (appNum, key) => {
    if (!appNum) {
      this.logger(`Application number is not valid ${appNum}`);
      return null;
    }

    const template = this.linkTemplates[key];
    if (!template) {
      throw new Error(`DataCollector.makeLink: Не определено направление ${key}`);
    }

    return this.buildUrl(template, { appNum });
  };

  buildUrl = (template, params) => {
    const path = template.path.replace(/:appNum/g, params.appNum);
