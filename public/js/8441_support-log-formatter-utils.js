...
const cleanLogRegEx = (
  ...
  const regex2 = /(\&|)gt\;/g;
  const regex3 = /(\&|)amp\;/g;
  const regex4_1 = /\&quot\;/g;
  const regex4 = /quot\;/g;
  const regex5 = /\#\d{2}\;/g;
  const regex6 = /<\?.*<return>/g;
  const regex7 = /<\/return.*>/g;
  const regexXmlNested = /<\?xml\s/g;
  ...
  if (correlationId && logType == 'СПР') {
    newStr = newStr.replace(regexXmlNested, '<?xxml_nested_ ');
  }
  ...
  !correlationId && logType == 'СПР'
    ? (document.getElementById(targetElementId).innerText = prettifyXml(newStr))
...
const cleanLog = (targetElementId, logStr, logType = null) => {
  ...
  console.log(
    `cleanLog: data: ${logCharArray[0] +
      logCharArray[1] +
      logCharArray[2]}... ${logType}`,
  );
      ...
      if (child.tagName == 'TBODY') {
        for (let tr of child.children) {
          if (trCoordinates.length > 0) {
            trCoordinates.forEach((coordinate) => {
    ...
    for (let tr of trs) {
      if (tr.rowIndex > 0) {
        ...
        if (window.location.href.split('/')[3] == 'support') {
          tr.children[
            cellIndex2
          ].innerHTML = `<a style="word-break: break-word" href="http://${currentHostName}:8440/support/${
          ...
          tr.children[
            cellIndex2
          ].innerHTML = `<a style="word-break: break-word" href="http://${currentHostName}:8440/kibana#${tr.getAttribute(
            'chainid',
          )}_group" style="color: #c4ecec">${val2}</a>`;
        }
        ...
        for (let key in log) {
          switch (key) {
            ...
            case 'message_size':
              ...
              tempCloneNode.children[
                getCellIndexByColumnName(key)
              ].children[0].href = `https://kibana.prod.ttl.tnt.fakeorg.ru/app/discover#/context/9e905f70-b88e-11ea-ad80-af2592106d05/${log['_id']}?_a=(columns:!(app_name,level,message,thread_name),filters:!())&_g=(filters:!())`;
              break;
            default:
              break;
          }
...
const shiftFixedBlock = element => {
  ...
  positionClassNames.forEach((position, idx) => {
    if (element.classList.contains(position)) {
      ...
      element.classList.toggle(shiftClassNames[idx]);
    } else if (element.classList.contains(shiftClassNames[idx])) {
      element.classList.toggle(position);
      element.classList.toggle(shiftClassNames[idx]);
    }
  });
