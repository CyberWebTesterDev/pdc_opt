...
SupportPageController = {
  ...
  _currentHostName: window.location.href
    .split('//')
    .join('')
    .split(':')[1],

  getCurrentLocalHostName: () => SupportPageController._currentHostName,
  ...
  BPMRequest: async (isAnalyze = false) => {
    ...
    let hostName = await SupportPageController.getCurrentLocalHostName();
    let urlBPM = `http://${hostName}:8440/frontrequest/bpmdata/${appnum}/${appStatus}`;
    // if (appStatus === 'Ошибка') {
    //   appStatus = 'ERR';
    // } else {
    //   appStatus = 'notERR';
  ...
  getIntegrationLogs: async (applicationNumber = null) => {
    ...
    if (!applicationNumber) {
      ...
      const url = `http://${SupportPageController._currentHostName}:8440/api/integrationlogs/${applicationNumber}`;
      try {
        ...
        if (result.redirected) {
          ...
          const parser = new DOMParser();
          doc = parser.parseFromString(html, 'text/html');
          const integrationTableHTML = doc.getElementById('intLogsRoot')
            .innerHTML;
          document.getElementById(
            'integratonLogsContainer',
          ).innerHTML = integrationTableHTML;
          SupportPageController.closeLoaderFlex();
          SupportPageController.callPopUp(
    ...
    if (result.ok) {
      ...
      if (result.redirected) {
        ...
        if (!additionalFlag) {
          ...
          await SupportPageController.loadELKControllerJS();
  ...
  adminkeyInfo: `<p style="color: white; font-size: 30px;">Интерфейс (толстый клиент) администратора для оперативного технического сопровождения АС РКК 2.0<br></p>
