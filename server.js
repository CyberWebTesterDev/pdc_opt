...
const routerIntegrationLogs = require('./routes/route-integration-logs');
const routerGetLocalHostName = require('./routes/route-get-local-hostname');
...
app.use('/api/integrationlogs/', routerIntegrationLogs);
app.use('/getlocalhostname/', routerGetLocalHostName);
    ...
    getProcessId(appnum)
      .then((process) => {
        if (process.length > 0 && process.length < 2) {
          ...
          const _uriCamundaBase = 'https://bpm.prod.ttl.tnt.fakeorg.ru/camunda/app/cockpit/default/#/process-instance/';
...
app.get('/support/:appnum', (req, res) => {
  ...
  let sessionMetaData = {
    pathAccessUnauthenticated: req.session.pathAccessUnauthenticated,
    userName: req.session.userName,
    personalData: req.session.personalData,
    remoteIPAddress: req.session.remoteIPAddress,
    dbDataCounter: req.session.dbDataCounter,
    bpmRequestCounter: req.session.bpmRequestCounter ? req.session.bpmRequestCounter : 0,
    integrationLogsRequestCounter: req.session.integrationLogsRequestCounter
      ? req.session.integrationLogsRequestCounter
      : 0,
    elkRequestCounter: req.session.elkRequestCounter ? req.session.elkRequestCounter : 0,
  };
